...
def import_poem1(cursor, poem_data, dynasty):
    """导入单首诗词"""
    try:
        # 提取数据
        ...
        author = zhconv.convert(author, 'zh-cn')
        # 处理内容（兼容不同格式）
        ...
        paragraphs = zhconv.convert(paragraphs, 'zh-cn')
        if not paragraphs:
            return False
        ...
        tags = zhconv.convert(tags, 'zh-cn')
        # 插入数据库
        cursor.execute('''
...
def import_poem(cursor, poem_data, dynasty):
    ...
    try:
        ...
        title = poem_data.get('title', '').strip()
        if title:
            title = zhconv.convert(title, 'zh-cn')  # 字符串直接转换[7,8](@ref)
        else:
            title = '无题'
        ...
        # 处理段落内容 - 正确处理列表类型[1](@ref)
        paragraphs = poem_data.get('paragraphs', [])
        if not paragraphs:
            paragraphs = poem_data.get('content', [])
        if not paragraphs:
            paragraphs = poem_data.get('para', [])
        ...
        for para in paragraphs:
            if isinstance(para, str):
                converted_para = zhconv.convert(para, 'zh-cn')
                converted_paragraphs.append(converted_para)
            else:
                converted_paragraphs.append(para)
        
        # 合并为纯文本内容
        content = ''.join(converted_paragraphs)
        
        if not content.strip():
            return False
        
        # 处理标签 - 同样正确处理列表类型[6](@ref)
        tags = poem_data.get('tags', [])
        converted_tags = []
        for tag in tags:
            if isinstance(tag, str):
                converted_tag = zhconv.convert(tag, 'zh-cn')
        ...
        ''', (
            ...
            dynasty,
            content,
            json.dumps(converted_paragraphs, ensure_ascii=False),
            json.dumps(converted_tags, ensure_ascii=False) if converted_tags else None
        ))
